...
- Erros de API: todos os componentes usam `apiFetchStrict` + `readableError` de `lib/api-error.ts` (portado do caracol-nf) — 422 do FastAPI vira mensagem legível (`campo: msg`) em vez de "[object Object]". `lib/api.ts` (replicado na suite) não é tocado.
- Formatação: `lib/i18n.ts` tem `fmtCurrency` blindado contra null e `fmtDateOnly` (data sem conversão de fuso), iguais ao caracol-nf.
- CI: `.github/workflows/ci.yml` roda `npm ci` + `npx tsc --noEmit` em push/PR pra `main`.
...
- **Fase 4.5** (atual): consolidação multi-moeda + conciliação de caixa. **Requer migration 043** (3 tabelas: `gerencial_fx_rates`, `gerencial_opening_balances`, `gerencial_remittances`).
  - **Cotação do dólar por mês** (Fechamento): informa US$ 1 = R$ X, fica salva; mês sem cotação herda a última cadastrada antes dele; sem nenhuma, usa R$ 5,60 (padrão, avisado no card). Card **"Resultado consolidado (R$)"** soma os dois lados (USD × cotação) no Mês e no Ano — responde como o mês/ano realmente fechou. Endpoints `/gerencial/fx-rates` (GET) e `/gerencial/fx-rate` (PUT).
