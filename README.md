...
- **Fase 4.7**: a aba **Fluxo de caixa** ganhou um **seletor de mês** no topo. A "Projeção de caixa por vencimento" e o bucket "Em atraso" continuam **a partir de hoje** por definição (o `/gerencial/cashflow` é ancorado em `today` e só aceita `months_ahead`, sem parâmetro de mês). O seletor controla as seções **por mês**: a nova **"Caixa realizado de \<mês\>"** (Recebido/Pago/Movimento do mês escolhido, com drill-down de títulos por data de liquidação — reusa `/gerencial/cashflow/items?month=`) e **"Saldos & conciliação"** (que perdeu o seletor próprio e passou a usar o do topo). Assim escolher, ex., MAIO mostra o caixa realizado de maio. _Para a própria projeção respeitar um mês passado seria preciso o backend aceitar `start=` em `/gerencial/cashflow` (hoje só `/forecast` aceita)._
